...
from ConvNN import Convolutional_NN
...
def train_test(dataframe_path,  num_rows, num_columns, num_channels):
...
def results(model, x_test, y_test):
    ...
    print("Model's final accuracy: " + acc + "%")
...
dataframe_path = '/Users/miguel.r/Desktop/featuresdf_refined.pkl'
...
x_train, x_test, y_train, y_test, dim, num_labels = train_test(dataframe_path, num_rows, num_columns, num_channels)
print('Label encoder classes:', y_train[1])

# Training the model:
history, model = Convolutional_NN(x_train, x_test, y_train, y_test, 50, 2, 
                                                            num_labels, num_rows, num_columns, num_channels)
...
for i in range (len(y_test)):
    if (np.argmax(y_test, axis=1)[i]) == 1:
        y_test_lb.append('heavy')
    if (np.argmax(y_test, axis=1)[i]) == 0:
        y_test_lb.append('light')
...
with open(saving_path+"_acc_"+acc+"_refined.json", "w") as json_file:
    json_file.write(model_json)
model.save_weights(saving_path+"_acc_"+acc+"_refined.h5")
